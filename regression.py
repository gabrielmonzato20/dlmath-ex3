...
def fit_regression_model(X, y):
    ...
    """
    learning_rate = 0.0001 # Pick a better learning rate 0.06 0.04 0.1
    num_epochs = 10000 # Pick a better number of epochs 4000 2000 20000 
    ...
    max_consecutive_epochs_no_improvement=5
    for epoch in range(1, num_epochs):
        loss = train_iteration(X, y, model, loss_fn, optimizer)
        if loss.item() >= previos_loss:
            consecutive_epochs_no_improvement += 1
        else:
            consecutive_epochs_no_improvement = 0

        # Check if there have been no improvements for the specified number of consecutive epochs
        if consecutive_epochs_no_improvement >= max_consecutive_epochs_no_improvement:
            print(f"Training stopped at epoch {epoch} due to no improvement in loss.")
            break
